...
export const DepartmentChangeHistory = ({ selectedEmployee }: { selectedEmployee: FormValues }) => {
	const [departmentHistory, setDepartmentHistory] = useState([]);

	useEffect(() => {
		const fetchDepartmentHistory = async () => {
			try {
				const response = await axios.get(`http://localhost:3000/employees/${selectedEmployee._id}/department-history`);
